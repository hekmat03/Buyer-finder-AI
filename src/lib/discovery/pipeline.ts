import type { SourceProvider, SourceSearchParams } from "@/lib/providers/source-provider";
...
import { matchService, type SupportedService } from "./service-match";
import type {
  DiscoveryRequest,
  DiscoveryResult,
  DiscoverySummary,
 ...
 *
 * Source providers are responsible only for obtaining source data.
 * This pipeline handles normalization, freshness, deduplication,
...
export async function runDiscoveryPipeline(
  ...
  try {
    rawCandidates = await provider.search(params);
  } catch (error) {
    summary.errors.push(
  ...
  for (const rawCandidate of rawCandidates) {
    try {
      const providerNormalized = provider.normalize(rawCandidate);
      const candidate = normalizeCandidate(providerNormalized);

      summary.normalized++;

      const freshness = assessFreshness(candidate.createdAt);
      ...
      const serviceMatch = matchService(
        `${candidate.title ?? ""}\n${candidate.text}`,
